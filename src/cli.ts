...
function validateOptions(opts: BotOptions): number {
  if (opts.from.trim().toUpperCase() !== "USDC") {
    throw new Error("This price-target reference currently requires --from USDC");
  }
...
function targetFromIntent(intent: ExecutionIntent, fallback: number): number {
  ...
  return typeof stored === "number" && Number.isFinite(stored) && stored > 0 ? stored : fallback;
}

function quoteEconomics(quote: CurrentQuote, inputUsdc: number, targetUsd: number) {
  if (quote.estimatedGasUsd === null) {
    throw new QuoteGuardError("Quote is missing estimated_gas_usd; refusing managed price-target execution");
  }
  if (quote.expiresAtMs <= Date.now() + 5_000) {
    throw new QuoteGuardError("Quote has 5 seconds or less remaining; request a fresh route");
  }
...
function printIntent(intent: ExecutionIntent, json: boolean): void {
  if (json) {
    console.log(JSON.stringify({ action: "execution_state", ...jsonIntent(intent) }));
    return;
  }
  console.log(
...
async function runBot(opts: BotOptions): Promise<void> {
  ...
  if (!opts.json) {
    console.log("Suwappu Price-Target Managed Swap Reference");
    console.log(`  Chain: ${opts.chain} | Buy: ${amountUsdc} USDC → ${opts.to}`);
    console.log(`  Reference trigger + conservative route target: < $${opts.target}`);
    console.log(`  Interval: ${opts.interval}s | Per-action cap: ${process.env.SUWAPPU_MAX_TRADE_USDC ?? DEFAULT_MAX_TRADE_USDC} USDC`);
    console.log(
      mode.kind === "managed"
    ...
    try {
        ...
        if (active) {
          ...
          try {
            const resumed = await runManagedExecution({
              ...
              getQuote: async () => (await qualifiedQuote({
                apiKey,
                terms: active.terms,
            ...
            if (error instanceof QuoteGuardError && active.phase === "prepared") {
              abandonPreparedExecution(active.id, error.message);
              if (!opts.json) console.log(`Prepared intent ${active.id} abandoned safely: ${error.message}`);
            } else {
              throw error;
            }
          }
      ...
      if (!shouldBuy(referencePrice, opts.target)) {
        if (opts.json) {
          ...
          console.log(`${opts.to} reference: $${referencePrice.toFixed(2)} (target: < $${opts.target})`);
        }
        retries = 0;
        await sleep(opts.interval * 1_000);
        continue;
      ...
      try {
        ...
        if (error instanceof QuoteGuardError) {
          if (opts.json) {
            ...
            console.log(`${opts.to} reference: $${referencePrice.toFixed(2)} < $${opts.target} — ROUTE BLOCKED`);
      ...
      if (mode.kind === "preview") {
        if (opts.json) {
          ...
          console.log(`${opts.to} reference: $${referencePrice.toFixed(2)} < $${opts.target} — WOULD BUY`);
          console.log(
      ...
      retries += 1;
      const rateLimited = error instanceof SuwappuRequestError
...
async function showExecutions(options: { reconcile?: boolean; json?: boolean }): Promise<void> {
...
const program = new Command()
  ...
  .description("Preview-first price-target managed-swap reference using Suwappu")
  .version("1.1.0")
  ...
  .option("--target <price>", "buy only below this conservative USD/unit price", Number.parseFloat, 2000)
  ...
  .option("--max-trades <n>", "terminal-success swaps this run before stopping", Number.parseInt, 1)
